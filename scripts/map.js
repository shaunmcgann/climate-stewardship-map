$(window).on('load', function() {
  ...
  function processAllPolygons() {
    ...
    while (p < polygonSettings.length && getPolygonSetting(p, '_polygonsGeojsonURL')) {
      ...
      polygonsLegend.onAdd = function(map) {
        ...
        content += '<form><label><div><input class="polygon-input" type="checkbox" value="' + p + ';0"><span><i class=""></i><span> '
          + getPolygonSetting(p, '_polygonsLegendTitle') + '</span></span></label>';
  ...
  function addLadderInteraction() {
    ...
    $('.ladder .polygon-input').change(function() {
  ...
  function customizeLadder() {
    ...
    solarForm.find('i').addClass('fa fa-circle').css('color', 'yellow');
    ...
    /* Transit-Oriented Zone -> Transportation */
    var tozLegend = $('.polygons-legend' + legendOrder['Transit-Oriented Zone']);
    var toz = tozLegend.children('form');
    ...
    var ctRiverLegend = $('.polygons-legend' + legendOrder['CT River']);
    var river = ctRiverLegend.children('form');
    river.find('i').addClass('fa fa-window-minimize').css('color', 'blue');
    waterLegend.append(river);
    ctRiverLegend.css('display', 'none');

    /* Trees by KNOX -> Public Parks */
    var ppLegend = $('.polygons-legend' + legendOrder['Public Parks']);
